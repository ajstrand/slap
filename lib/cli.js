...
module.exports = function (options) {
  ...
  return Promise.all([
    ...
    if (userDir)
      opts = _.merge(
        { logger: { file: path.resolve(userDir, package.name + ".log") } },
        opts
      );
    ...
    if (!opts.config)
      fs.createReadStream(path.join(__dirname, "..", "default-config.ini")) // if a user config doesn't exist, make one by copying a template
