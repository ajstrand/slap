...
class Slap {
  constructor(opts) {
    ...
    if (!(self instanceof Slap))
      return new Slap(opts);

    if (!Slap.global)
      Slap.global = this;
    if (opts.screen && !opts.screen.slap)
      opts.screen.slap = self;
  ...
  _initHandlers() {
    ...
    self.on('element keypress', function (el, ch, key) {
      ...
      while ((focused = focused.parent) && !(focusedBinding = BaseWidget.prototype.resolveBinding.call(focused, key)))
        ;
      if (focusedBinding)
        logLine += " (bound to " + focusedBinding + " on " + util.typeOf(focused) + ")";
      ...
      switch (binding) {
        ...
        case 'quit':
          if (!self._panesBlockingQuit) {
            ...
            if (currentPane)
              currentPane.close(); // ensures panes[0] doesn't steal focus
    ...
    self.on('element close', _.debounce(function (el, i) {
      ...
      if (nextPane && nextPane.isAttached())
        nextPane.setCurrent();
    }));
  ...
  _initPlugins() {
    ...
    return Slap.getUserDir().then(function (userDir) {
      console.log("plugins are disabled at the moment");
      // return clap({
      //   val: self,
      //   module: require.main,
      //   keyword: 'slap-plugin',
  ...
  static getUserDir() {
    var userDir = util.resolvePath('~/.' + name);
    return mkdirp_promise_edition(userDir).return(userDir);
  }
}
Slap.global = null;
