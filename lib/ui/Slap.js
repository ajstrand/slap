...
Slap.prototype._initHandlers = function () {
  ...
  self.on("element keypress", function (el, ch, key) {
    ...
    while (
      ...
      !(focusedBinding = BaseWidget.prototype.resolveBinding.call(focused, key))
    );
    if (focusedBinding)
      logLine +=
...
Slap.prototype._initPlugins = function () {
  ...
  return Slap.getUserDir()
    ...
    .map(function (obj) {
      return obj.promise
        .then(function () {
          util.logger.info("loaded plugin " + obj.plugin);
        })
        .catch(function (err) {
          util.logger.error(
            "failed loading plugin " + obj.plugin + ": " + (err.stack || err)
          );
        });
    });
};
