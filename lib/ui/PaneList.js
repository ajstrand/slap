...
class PaneList extends Pane {
  constructor(opts) {
    ...
    if (!(self instanceof PaneList)){
      return new PaneList(opts);
    }

    //new Pane.call(self, _.merge({}, Slap.global.options.paneList, opts));
    self.options = {...self, ..._.merge({}, Slap.global.options.paneList, opts) }
    ...
    self.options = {...self.list, ...listOpts}
    ...
    ['adopt', 'remove'].forEach(function (evt) {
      slap.on('element ' + evt, function (parent, child) {
        if (child instanceof Pane)
          setImmediate(function () { self.update(); });
      });
    });
