...
class FindForm extends BaseFindForm {
  constructor(opts) {
    ...
    self.options = {...self, ..._.merge(
      ...
      //self.findField = {left: this._label.length }
  ...
  _initHandlers() {
    ...
    self.on(
      "find",
      _.throttle(function (pattern, direction) {
        ...
        editor._updateContent();
        //FIXME: this is a hack harcoding the find throttle value 
        // since options arent inherited/created correctly
        // need to fix
      }, 150) //self.options.perf.findThrottle)
    );
